...
- (void)configureView
{
    ...
    if (self.detailItem) {
        _author = (Author*) self.detailItem;
        self.title = _author.name;
        self.managedObjectContext = _author.managedObjectContext;
    }
}
...
- (NSFetchedResultsController *)fetchedResultsController
{
    ...
    // Delete cache -- may remove in production
    ...
    // Set the predicate
    NSPredicate* predicate = [NSPredicate predicateWithFormat:@"whoWrote.name = %@", _author.name];
...
-(void)addObjectController:(id)addObjectController didUpdate:(NSDictionary *)dictionary{
    ...
    [newDict setObject:_author.name forKey:@"authorName"];
    [Book bookWithDictionary:newDict inManagedObjectContext:context];
}
